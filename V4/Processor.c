...
void Processor_ShowTime(char); //[MOISES V2] EJ 1
...
void Processor_InstructionCycleLoop() {
	...
	while (!Processor_PSW_BitState(POWEROFF_BIT)) {
		...
		if (interruptLines_CPU && !Processor_PSW_BitState(INTERRUPT_MASKED_BIT)){ //[MOISES V2] EJ 3
	...
	switch (operationCode) {
		...
		case DIV_INST: 
			if (operand2 == 0)
				...
				Processor_RaiseException(DIVISIONBYZERO); //MOISES. EJ 1 V4
			else {
				registerAccumulator_CPU=operand1 / operand2;
				registerPC_CPU++;
			}
		...
		// Instruction HALT
		case HALT_INST:
			if (Processor_PSW_BitState(EXECUTION_MODE_BIT)) { //[MOISES V1] EJ 16
				Processor_ActivatePSW_Bit(POWEROFF_BIT);
			}
			else {
				//Processor_RaiseInterrupt(EXCEPTION_BIT);
				Processor_RaiseException(INVALIDPROCESSORMODE); //MOISES. EJ 1 V4
			}
		...
		// Unknown instruction
		default : 
			Processor_RaiseException(INVALIDINSTRUCTION); //[MOISES V4] EJ 3 
			registerPC_CPU++;
			break;
	}
	...
	int i;
		...
		for (i=0;i<INTERRUPTTYPES;i++)
			...
			if (Processor_GetInterruptLineStatus(i)) {
				...
				Processor_ActivatePSW_Bit(INTERRUPT_MASKED_BIT); //Activa el bit de interrupciones enmascaradas  EJ3 de la V2 
...
char * Processor_ShowPSW(){
	...
	if (Processor_PSW_BitState(INTERRUPT_MASKED_BIT))
		pswmask[tam - INTERRUPT_MASKED_BIT] = 'M'; //[MOISES V2] EJ 3
	return pswmask;
}
...
//Devuelve valor del registro de proprosito general B.
