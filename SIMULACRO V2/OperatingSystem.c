...
void OperatingSystem_TerminateProcess() {
	...
	if (numberOfNotTerminatedUserProcesses==0) {
		if (executingProcessID==sipID) {
			...
			ComputerSystem_DebugMessage(99,SHUTDOWN,"The system will shut down now...\n");
			OperatingSystem_ShowTime(SYSPROC);
			ComputerSystem_DebugMessage(551, SYSPROC, userThrows);

			OperatingSystem_ShowTime(SYSPROC);
			ComputerSystem_DebugMessage(552, SYSPROC, daemonThrows);
...
void OperatingSystem_SYSCALL_SLEEP() {
	...
	if (newProcess != NOPROCESS) { //Si hab�a procesos en la cola de listos 
		...
		else if (processTable[newProcess].queueID == DAEMONSQUEUE) {
			daemonThrows += 1; 
		}
