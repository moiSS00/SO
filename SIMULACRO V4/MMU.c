...
// The MAR register
int registerMAR_MMU; //A utilizar en EJ1 d DIRECCION LOGICA 

// The CTRL register
int registerCTRL_MMU; //A utilizar en EJ1 d
	...
	switch (registerCTRL_MMU) {
  	case CTRLREAD:
			if (Processor_PSW_BitState(EXECUTION_MODE_BIT)){ // Protected mode
				if ((registerMAR_MMU < MAINMEMORYSIZE) && (registerMAR_MMU >= 0)){ //[MOISES V4] EJ 0
				...
				if ((registerMAR_MMU >= 0) && (registerMAR_MMU < registerLimit_MMU)) { //[MOISES V4] EJ 0
