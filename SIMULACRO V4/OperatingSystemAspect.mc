 ...
 while (newProgram == 1)
 {
  ...
  if (PID < 0) {
   ...
   if (PID == -3 || PID == -5 || PID == -4) {
    ...
    ComputerSystem_DebugMessage(550,'p',programList[i]->executableName);
   }
  }
...
int OperatingSystem_MaxProirity() {
 ...
 if (newProcess != -1) {
  ...
  else if ((queueID == USERPROCESSQUEUE) && (processTable[newProcess].queueID == DAEMONSQUEUE)) {
   return -1;
  }
  else {
   return newProcess;
