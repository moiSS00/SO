...
int OperatingSystem_LongTermScheduler() {
	...
	while (newProgram == YES) //[MOISES V3] EJ 2 
	{
		...
		if (PID < 0) {
			...
			// Simulacro examen V4 
			if (PID == NOFREEENTRY || PID == MEMORYFULL || PID == TOOBIGPROCESS) {
				programList[i]->arrivalTime += programList[i]->arrivalTime % intervalBetweenInterrupts + 5 * intervalBetweenInterrupts;
				Heap_add(i,arrivalTimeQueue,QUEUE_ARRIVAL,&numberOfProgramsInArrivalTimeQueue,numberOfProgramsInArrivalTimeQueue+1);
				OperatingSystem_ShowTime(SYSPROC);
				ComputerSystem_DebugMessage(550,SYSPROC,programList[i]->executableName);
			}
		}
